...
通用AI腔（套话、教科书开头、稻草人对比、营销号腔等九组）见 `~/.claude/rules/no_ai_style.md`，本文件不复述。

以下是卡兹克本人的风格禁忌，比通用规则更严，冲突时以这里为准：

1. **过度结构化**：不用bullet point罗列观点，不大量加粗。他的文章绝大多数时候是连续段落
2. **标点禁令**：不使用冒号「：」，用逗号代替；不使用破折号；不使用任何双引号，需要引用或强调时用「」或者直接不加引号
3. **他的个人踩雷词**（通用清单里没有的）：「说白了」「意味着什么？」「这意味着」「本质上」「换句话说」「不可否认」
4. **假设性例子**：「比如有一次……」这种编造场景是大忌。要用「就像我今天正在搞的xxx」这种真实的
5. **空泛工具名**：不说「AI工具」「某个模型」，要说具体名字，比如Claude Code、Codex
