...
| 打印/存档PDF | 用户明确要A4打印或归档格式时 | `references/pdf_rendering.md`（视觉特化）加 `~/.claude/skills/pdf-publisher/routes/html.md`（技术链路），两份都要读 |

其他格式（.xlsx、.md、单图）仅在用户明确要求时使用。

**第二步 气质（由设计判读决定）**：动手前先输出一行设计判读（读者是谁、什么场合、承诺什么视觉方向），配色、字体、构图全部为该方向服务。读取使用者的审美偏好存档（memory或项目CLAUDE.md）作为边界约束；禁止套用上一份报告的成品风格当默认，禁止无判读即兴。流程细则、密度规则与质量闸见`references/html_report.md`。
...
2026-07-25从`personal-ai-os/checklist.md`的B5并入。逐条过，不过就修。

1. **每个图表回答一个具体问题**，说不出问题的图删掉
2. **结论有数据支撑**，数字标注口径（时间范围、过滤条件）
3. **Insight可行动**：说清楚谁、该做什么、预期什么变化

报告是HTML产出时另加三条：中文排版和字体按`~/.claude/rules/cn_typography.md`；图表要有图例和轴标签、不靠颜色单维区分数据系列、空态错态加载态都有方案；长表格给横向滚动容器和表头吸顶。

## 输出语言规范

生成报告正文、图注、建议文案时遵守`~/.claude/rules/no_ai_style.md`，本文件不复述其条款（2026-07-25从抄全文改为路径指针，抄件会过期且没人核对）。

分析报告特别容易犯的两条：结论段写成道理升华（第八组「宣言式原则句」），以及用「不是XX而是XX」造假对比撑结论（第九组）。
