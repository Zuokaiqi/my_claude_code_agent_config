# 中文页面字体池（西式设计skill的中文补丁）

## 何时读

做**中文页面**时必读，不分页面类型：营销页、产品UI、看板、课件deck都算。用impeccable、frontend-slides等西式设计skill时尤其必读，它们的字体规则建立在拉丁字体上（Geist、Satoshi、Cabinet Grotesk等），全部不覆盖中文，直接套用会退化成系统默认宋体黑体。本文件把通用字体原则（display要有性格、正文要中性耐读、一个项目一套搭配）翻译成中文可用的字体池。
...
- font-family里西文字体放中文字体**前面**，让拉丁字符走西文字体：`"Geist", "Noto Sans SC", sans-serif`。这样选定的西文display字体仍然生效，中文自动落到中文字体
...
## 中文断行（2026-07-25从ui_engineering_baseline并入）

- 大标题控制断行，手动`<br>`或`text-wrap:balance`，**禁止单字孤行**
- 标题字号上限按最长一行在最窄容器宽度验算，不许凭感觉给clamp上限
- 长prose用`text-wrap:pretty`减少孤字

这两条西式设计里不存在，所有西式skill都不会提醒。
...
装饰性线条、路径、图形不得与文字块重叠，绘制前先核对文字占位。这条是实际踩坑来的，西式设计skill都不提醒。

## 产品UI和营销页的审美来源

产品UI读 `~/.claude/skills/impeccable/SKILL.md` 加 `reference/product.md`，它自带完整禁令清单。营销页不加载任何审美方法论skill，按内容和受众直接做（2026-07-24盲测：什么都不读的裸模型在营销页上得分最高）。
